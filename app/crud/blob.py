...
from app.models.blob import BlobFilterParams, Blob, BlobInCreate, BlobInDb
...
async def crud_get_all_blobs(db: AsyncIOMotorClient, filters: BlobFilterParams) -> List[Blob]:
    blobs: List[Blob] = []
    base_query = {}
...
async def crud_create_blob(db: AsyncIOMotorClient, blob: BlobInCreate, bucket_name: str, update: bool = False) -> BlobInDb:
    ...
    if not update:
        row = await db[DATABASE_NAME][COLLECTION].insert_one(data_blob.dict())
        ...
        updated_at = await db[DATABASE_NAME][COLLECTION].update_one({'path': data_blob.path},
                                                                    {'$set': data_blob.dict()})
