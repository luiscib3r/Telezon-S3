...
from app.core.config import CID, SESSION_STRING, TELEGRAM_API_ID, TELEGRAM_API_HASH
...
class TelegramAccountStorage(Storage):
    async def put_file(self, file: bytes, filename: str) -> str:
        file_name = os.path.join(
            tempfile.gettempdir(), f'telezon_{filename}')
    ...
    async def get_file(self, file_id: str):
        file_name = os.path.join(
            tempfile.gettempdir(), f'telezon_{file_id}')
